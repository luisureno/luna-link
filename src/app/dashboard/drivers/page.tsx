                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                              ...
                              {detail.tickets.length === 0 ? (
                                <p className="text-sm text-gray-400">No loads submitted today.</p>
                              ) : (
                            ...
                            <div>
                              ...
                              {!detail.inspection ? (
                                <p className="text-sm text-gray-400">Not completed today.</p>
                              ) : (
                                <div>
                                  <div className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2 py-1 rounded-full mb-3 ${detail.inspection.overall_status === 'passed' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                    {detail.inspection.overall_status === 'passed' ? '✓ All Clear' : '⚠ Issues Found'}
                                  </div>
                                  <div className="space-y-1.5">
                                    {(detail.inspection.items as any[]).map((item: any) => (
                                      <div key={item.id} className={`flex items-start gap-2 bg-white border rounded px-3 py-2 ${!item.passed ? 'border-red-200' : 'border-gray-200'}`}>
                                        ...
                                        <div className="flex-1 min-w-0">
                                          ...
                                          {item.photo_url && <a href={item.photo_url} target="_blank" rel="noreferrer" className="text-xs text-blue-600 hover:underline" onClick={e => e.stopPropagation()}>View Photo</a>}
                                        </div>
                                      </div>
                                    ))}
                                  </div>
                            ...
                            <div>
                              <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
                                Fuel Stops ({detail.fuelLogs.length})
                                {detail.fuelLogs.length > 0 && (
                                  <span className="ml-2 text-gray-400 normal-case font-normal">
                                    — ${detail.fuelLogs.reduce((s, f) => s + Number(f.total_cost), 0).toFixed(2)} total
                                  </span>
                                )}
                              </h3>
                              {detail.fuelLogs.length === 0 ? (
                                <p className="text-sm text-gray-400">No fuel stops today.</p>
                              ) : (
                                <div className="space-y-1.5">
                                  {detail.fuelLogs.map(f => (
                                    <div key={f.id} className="flex items-center justify-between bg-white border border-gray-200 rounded px-3 py-2">
                                      ...
                                      <div className="flex items-center gap-2">
                                        ...
                                        {f.receipt_url && (
                                          <a href={f.receipt_url} target="_blank" rel="noreferrer" className="text-xs text-blue-600 hover:underline" onClick={e => e.stopPropagation()}>
                                            Receipt
                                          </a>
                                        )}
                                      </div>
